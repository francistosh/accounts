...
echo'<td style="text-align: right">From:&nbsp; <b>'.$start.'</b>&nbsp; to &nbsp;<b>'.$end.'</b></td></tr>';
echo '<tr><td style="'.$display.'">Sabil No: &nbsp; '.strtoupper($sabil).'&nbsp;&nbsp; House No:&nbsp;'.ucfirst($hseno).'&nbsp;&nbsp; Masool Name:&nbsp;</td><td style="text-align:Center">  </td></tr>';
echo '<tr></tr>'; 
echo '</table></div><hr />';

echo '<table id="statements" class="table table-bordered" style="font-size:12px;" >';
...
echo '<tbody>';
      ...
      //opening balance-b/bf
        ...
        $qr="SELECT idate as date,invno as docno,incacc,rmks,ts,IF(isinvce='1',amount,'')as debitamt,IF(isinvce='0',amount,'')as creditamt,if(isinvce='1','Invoice','Credit Note') as doctype , ' ' as pmode, subacc FROM invoice WHERE sabilno LIKE '$sabil' AND estId LIKE '$estId' AND idate BETWEEN '$start1' AND '$end1' AND incacc = '".$indata[$k]['incacc']."' AND opb = '0' UNION 
            ...
            SELECT rdate as date,recpno as docno,'0' as incacc,rmks,ts,IF(amount<0,-1*amount,'')as debitamt,IF(amount>0,amount,'')as creditamt,if(tno>0,'PD','') as doctype, IF(pmode='CASH',pmode,concat(pmode,' ','NO:',' ',chqno,' ',chqdet)) AS pmode,'' as subacc FROM pdchqs WHERE rdate BETWEEN '$start1' AND '$end1' AND sabilno LIKE '$sabil' AND est_id LIKE '$estId' AND incacc = '".$indata[$k]['incacc']."' AND banked = '0' UNION 
...
echo'<tr><td rowspan=5 colspan=5 style="padding-top: 70px;"><h7>Please check your balance and bring any discrepancies within 15 days</h7>
...
echo'<tr><td>Paid</td><td align="right">'.number_format($paidtotal,2).'</td></tr>';
echo'<tr><td>PD Cheques</td><td align="right">'.number_format($pdtotal,2).'</td></tr>';
