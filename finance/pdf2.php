    ...
    <body>  
      <?php
        ...
        //$toStatement= $start."&nbsp;TO&nbsp;".$end."<br/>Sabil No :<font style='text-transform:uppercase'>".$sabil."</font><br/>HOF Ejamaat No :".$mumin->get_MuminHofEjnoFromSabilno($sabil); 
            ...
            $invrc=$mumin->getdbContent("SELECT * FROM estate_debtors WHERE dno = '$debtor' LIMIT 1"); 
        ...
        <h1><font size="3"> <?php echo  $_SESSION['estatefullname']?> , &nbsp<?php echo $_SESSION['sector'] ?> </font><br>
            <span style="font-size:80%;">P.O. Box 81766-80100, Tel: 020-2040372, Mombasa, Kenya
          ...
          <table>
              ...
              <tr>
                  ...
                  <td style="text-align:Center"><span><b>SECTOR :&nbsp;&nbsp;<?PHP echo $msector; ?></span>  </td>
              </tr>
        ...
        <table style="border-collapse: collapse;">  
           ...
           $qry3= "SELECT SUM(amount) as bbfamnt from (SELECT SUM(IF(isinvce='1',amount,-1*amount)) as amount  FROM estate_invoice WHERE sabilno LIKE '$sabil' AND estId LIKE '$estId'  AND idate < '$start' UNION 
               SELECT (SUM(amount))*-1 as amount  FROM estates_recptrans WHERE sabilno LIKE '$sabil' AND est_id LIKE '$estId' AND rdate < '$start') t1";
        ...
        $qr="SELECT idate as date,invno as docno,rmks,IF(isinvce='1',amount,'')as debitamt,IF(isinvce='0',amount,'')as creditamt,if(isinvce='1','Invoice','Credit Note') as doctype , ' ' as pmode FROM estate_invoice WHERE sabilno LIKE '$sabil' AND estId LIKE '$estId' AND idate BETWEEN '$start' AND '$end' UNION 
            SELECT rdate as date,recpno as docno,rmks,IF(amount<0,-1*amount,'')as debitamt,IF(amount>0,amount,'')as creditamt,if(tno>0,'Receipt','') as doctype, IF(pmode='CASH',pmode,concat(pmode,' ','NO:',' ',chqno,' ',chqdet)) AS pmode FROM estates_recptrans WHERE rdate BETWEEN '$start' AND '$end' AND sabilno LIKE '$sabil' AND est_id LIKE '$estId' ORDER BY date";
      ...
      //opening balance-b/bf
            $qr4= "SELECT SUM(amount) as bbfamnt from (SELECT SUM(IF(isinvce='1',amount,-1*amount)) as amount  FROM estate_invoice WHERE dno = '$debtor' AND estId LIKE '$estId'  AND idate < '$start' AND sector=' ' UNION 
               SELECT (SUM(amount))*-1 as amount  FROM estates_recptrans WHERE dno = '$debtor' AND est_id LIKE '$estId' AND rdate < '$start' AND sector=' ') t1";
            $result4=$mumin->getdbContent($qr4);           
            $balance_bf= $result4[0]['bbfamnt'];
   
      //show the breakdown from recptrans table and Invoice Table  
           
             $qr="SELECT idate as date,invno as docno,rmks,IF(isinvce='1',amount,'')as debitamt,IF(isinvce='0',amount,'')as creditamt,if(isinvce='1','Invoice','Credit Note') as doctype, ' ' as pmode FROM estate_invoice WHERE dno = '$debtor' AND estId = '$estId' AND idate BETWEEN '$start' AND '$end' AND sector=' ' UNION 
            SELECT rdate as date,recpno as docno,rmks,IF(amount<0,-1*amount,'')as debitamt,IF(amount>0,amount,'')as creditamt,if(tno>0,'Receipt','')as doctype, IF(pmode='CASH',pmode,concat(pmode,' ','NO:',' ',chqno,' ',chqdet)) AS pmode FROM estates_recptrans WHERE rdate BETWEEN '$start' AND '$end' AND dno = '$debtor' AND est_id LIKE '$estId' AND sector=' ' ORDER BY date";
                 $q=$mumin->getdbContent($qr);   
                }elseif ($_SESSION['grp']=='EXTERNAL') {
                $qr4= "SELECT SUM(amount) as bbfamnt from (SELECT SUM(IF(isinvce='1',amount,-1*amount)) as amount  FROM estate_invoice WHERE dno = '$debtor' AND estId LIKE '$estId'  AND idate < '$start' AND sector='".$_SESSION['sector']."' UNION 
               SELECT (SUM(amount))*-1 as amount  FROM estates_recptrans WHERE dno = '$debtor' AND est_id LIKE '$estId' AND rdate < '$start' AND sector='".$_SESSION['sector']."') t1";
            $result4=$mumin->getdbContent($qr4);           
            $balance_bf= $result4[0]['bbfamnt'];
   
      //show the breakdown from recptrans table and Invoice Table  
           
             $qr="SELECT idate as date,invno as docno,rmks,IF(isinvce='1',amount,'')as debitamt,IF(isinvce='0',amount,'')as creditamt,if(isinvce='1','Invoice','Credit Note') as doctype, ' ' as pmode FROM estate_invoice WHERE dno = '$debtor' AND estId = '$estId' AND idate BETWEEN '$start' AND '$end' AND sector='".$_SESSION['sector']."' UNION 
            SELECT rdate as date,recpno as docno,rmks,IF(amount<0,-1*amount,'')as debitamt,IF(amount>0,amount,'')as creditamt,if(tno>0,'Receipt','')as doctype,IF(pmode='CASH',pmode,concat(pmode,' ','NO:',' ',chqno,' ',chqdet)) AS pmode FROM estates_recptrans WHERE rdate BETWEEN '$start' AND '$end' AND dno = '$debtor' AND est_id LIKE '$estId' AND sector='".$_SESSION['sector']."' ORDER BY date";
                 $q=$mumin->getdbContent($qr);   
