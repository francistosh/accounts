...
<button id="closestmnt" class="sexybutton sexymedium sexyyellow" onclick="window.history.back();"><span><span><span class="cancel">Back</span></span></span></button>
...
<br />
 ...
 &nbsp;&nbsp;<button class="send-cancel" id="cancelb" >Cancel</button>  
        ...
        <?php
        $param='sabil';
        $start1= date('Y-m-d',strtotime($_POST['multifrmdate']));
		$start = $_POST['multifrmdate'];
        $end1= date('Y-m-d',  strtotime($_POST['multitodate']));
		$end = $_POST['multitodate'];
        $estId = $id;
        $account=$_POST['multincmeacc'];
       
        if($param=="sabil"){
                 
        $sabil=$_POST['multiselctivesabno'];
...
echo '<div id="printableArea" style="width: 640px;margin-left:auto;margin-right: auto">';
//echo ''
...
echo '<td hidden="true"><input type="text" id="paramid" value="'.$param.'"></input><input type="text" id="startid" value="'.$start1.'"></input> <input type="text" id="endid" value="'.$end1.'"></input>
                        <input type="text" id="sabilid" value="'.$sabil[$s].'"></input><input type="text" id="accountid" value="'.$account.'"></input><input type="text" id="debtorid" value="'.@$debtor.'"></input>
                        <input type="text" id="emailid" value="'.$email.'"></input>    </td>';
echo '<td colspan="2"> <p> <i><font style="font-size:20px;text-align: left;font-weight:bold;font-family: Cambria,Georgia,serif;">&nbsp&nbsp; Anjuman-e-Burhani  </font></i> </p> <span style="font-size:10px">'.$_SESSION['details'].'</span> </td><td><span style="float:right;font-size: 8px; padding-right: 30px"><b>'.$_SESSION['dptname'].'</b></span><br><img src="../assets/images/gold new logo.jpg" style="float:right" height="80" width="80" /></img></td>';
echo '</tr> ';
echo '</table>';
echo '<div align="right"><font size="4"><b>Statement</font></b> </div>';
echo '<hr />';
echo '<div><table id="report" style="font-size:12px;padding:10px;">';
echo '<tr><td>Account Name: &nbsp; <b>'.$statementRecevee.' </b>&nbsp;&nbsp;&nbsp; Tel No : '.$debtelno.'</td></tr>';
echo '<tr><td style="'.$display.'">Sabil No: &nbsp; '.strtoupper($sabil[$s]).'&nbsp;&nbsp; House No.&nbsp;'.ucfirst($hseno).'</td><td style="text-align:Center">  </td></tr>';
echo '<tr><td style="text-align: right">From:&nbsp; <b>'.$start.'</b>&nbsp; to &nbsp;<b>'.$end.'</b></td></tr>'; 
echo '</table></div>';
echo '<hr />';
...
echo '<tbody>';
      ...
      //opening balance-b/bf
           ...
           $qry56= $mumin->getdbContent("SELECT SUM(amount) as bbfamnt , tno from (SELECT SUM(IF(isinvce='1',amount,-1*amount)) as amount,tno  FROM invoice WHERE sabilno LIKE '$sabil[$s]' AND estId LIKE '$estId'  AND incacc = '".$indata[$k]['incacc']."' AND opb = '1'  ) t1");
            ...
            //balance-b/bf
           $qry3= "SELECT SUM(amount) as bbfamnt , tno from (SELECT SUM(IF(isinvce='1',amount,-1*amount)) as amount,tno  FROM invoice WHERE sabilno LIKE '$sabil[$s]' AND estId LIKE '$estId'  AND idate < '$start1' AND incacc = '".$indata[$k]['incacc']."' AND opb = '0' UNION 
               SELECT (SUM(amount))*-1 as amount,tno  FROM recptrans WHERE sabilno LIKE '$sabil[$s]' AND est_id LIKE '$estId' AND rdate < '$start1' AND incacc = '".$indata[$k]['incacc']."'  UNION
               SELECT (sum(dramount) - sum(cramount)) as amount,tno FROM bad_debtsmbrs WHERE acc = '$sabil[$s]' AND deptid = '$estId' AND jdate < '$start1' AND incacc = '".$indata[$k]['incacc']."' and tbl = 'M' ) t1";
        ...
        $qr="SELECT idate as date,invno as docno,incacc,rmks,ts,IF(isinvce='1',amount,'')as debitamt,IF(isinvce='0',amount,'')as creditamt,if(isinvce='1','Invoice','Credit Note') as doctype , ' ' as pmode, subacc FROM invoice WHERE sabilno LIKE '$sabil[$s]' AND estId LIKE '$estId' AND idate BETWEEN '$start1' AND '$end1' AND incacc = '".$indata[$k]['incacc']."' AND opb = '0' UNION 
            SELECT rdate as date,recpno as docno,'0' as incacc,rmks,ts,IF(amount<0,-1*amount,'')as debitamt,IF(amount>0,amount,'')as creditamt,if(tno>0,'Receipt','') as doctype, IF(pmode='CASH',pmode,concat(pmode,' ','NO:',' ',chqno,' ',chqdet)) AS pmode,'' as subacc FROM recptrans WHERE rdate BETWEEN '$start1' AND '$end1' AND sabilno LIKE '$sabil[$s]' AND est_id LIKE '$estId' AND incacc = '".$indata[$k]['incacc']."' UNION 
             SELECT jdate as date, jno as docno,incacc,rmks,ts,if(dramount <> '0.00',dramount,'') as debitamt,if(cramount <> '0.00',cramount, '') as creditamt,'Journal Entry' as doctype,' ' as pmode,'' as subacc  FROM bad_debtsmbrs WHERE acc = '$sabil[$s]' AND deptid = '$estId' AND jdate BETWEEN '$start1' AND '$end1' AND  incacc = '".$indata[$k]['incacc']."' AND tbl = 'M'  ORDER BY date,ts ASC ";
...
echo'<tr><td colspan=4>Totals</td><td style="text-align: right;font-size:12px;padding-right:10px">'.number_format($dbtotal,2).'</td><td style="text-align: right;font-size:12px;padding-right:10px">'.number_format($crdtotal,2).'</td><td style="text-align: right;font-size:12px;padding-right:10px">'.number_format($balnce,2).'</td></tr>';
echo '<td colspan=7><br></br>  </td>';
